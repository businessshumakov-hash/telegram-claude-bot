...
async function autoSaveConversationSummary() {
  ...
  const prompt = `Проаналізуй діалог між Богданом і Міею та витягни НОВІ факти для довготривалої пам'яті.
...
Правила:
- Лише конкретні факти: плани, рішення, вподобання, стан проектів
- НЕ дублюй вже відоме
- НЕ зберігай: поточний час, технічні деталі Міі, одноразові минулі події
- Якщо нічого нового — поверни {"clients":[],"habits":[],"plans":[],"facts":[]}

Категорії: clients (клієнти/проекти), habits (звички/вподобання), plans (майбутні плани), facts (особисті факти)
...
async function extractAndSaveMemory(userMessage, assistantMessage) {
  ...
  const prompt = `Проаналізуй діалог і витягни НОВІ конкретні факти про Богдана для довготривалої пам'яті.
...
Правила:
- Запам'ятовуй лише конкретні факти (плани, вподобання, рішення, стан справ)
- НЕ зберігай: поточний час, технічні деталі Міі, загальні розмови, запити до ClickUp
- НЕ дублюй вже відоме
- Якщо нічого нового — поверни {"clients":[],"habits":[],"plans":[],"facts":[]}
